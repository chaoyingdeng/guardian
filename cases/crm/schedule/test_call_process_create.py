...
@allure.title('拜访2.0新建拜访流程')
def start(instance, user_info):
    # 获取拜访模板列表和机构列表
    temp_list = instance.crm.get_call_template_list(user_info, Times().today)
    institution_list = instance.crm.get_call_institution_list(user_info).get('records')
    ...
    customer_list_info = instance.crm.get_customer_by_institution(institution_id, user_info).get('records')
    ...
    dict_list = instance.crm.get_call_template_detail(template_id).get('displayFieldConfig').get('basic').get('fields')
    ...
    call_type = instance.saas.get_dict_detail_by_id(call_type_id)[0].get('dictionaryEntries')[0].get('value')
    call_purpose = instance.saas.get_dict_detail_by_id(call_purpose_id)[0].get('dictionaryEntries')[0].get('value')
